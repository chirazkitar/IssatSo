...
import { modulesAPI, usersAPI } from '../../api/apifetch';
...
export default function ModulesPage() {
  ...
  useEffect(() => {
    Promise.all([modulesAPI.list(), usersAPI.teachers()])
      .then(([mods, tchs]) => {
        setModules(mods);
        setTeachers(tchs);
        // Pre-fill assignment dropdowns with current teacher
        const init = {};
  ...
  async function assign(moduleId) {
    ...
    try {
      ...
      toast(
        teacher
          ? `Module affecté à ${teacher.first_name} ${teacher.last_name}`
          : 'Affectation retirée',
        'success'
      );
    } catch (err) {
  ...
  const filteredModules = search
    ? modules.filter((m) => `${m.name} ${m.code}`.toLowerCase().includes(search.toLowerCase()))
    : modules;
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }} className="fade-up-1">
        ...
        <Chip variant="chip-green">
          {modules.filter((m) => m.teacher_id).length} assignés
        </Chip>
        <Chip variant="chip-red">
          {modules.filter((m) => !m.teacher_id).length} non assignés
        </Chip>
      </div>
        ...
        return (
          <div key={sem} style={{ marginBottom: 22 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }} className="fade-up">
              <Chip variant="chip-accent" style={{ padding: '6px 14px', fontSize: 12 }}>
                 Semestre {sem}
              </Chip>
                      ...
                      <tr key={m.id}>
                        ...
                        <td>
                          {m.teacher_name ? (
                            <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                              <Avatar
                                firstName={m.teacher_name.split(' ')[0]}
                                lastName={m.teacher_name.split(' ')[1] || ''}
                                size="sm"
                              />
                        ...
                        <td>
                          <select
                            ...
                            onChange={(e) =>
                              setAssignments((a) => ({ ...a, [m.id]: e.target.value }))
                            }
                        ...
                        <td>
                          ...
                          >
                            {saving[m.id] ? <Spinner size={12} /> : 'Affecter'}
                          </button>
                        </td>
                      </tr>
                    ))}
