...
export default function UsersAdmin() {
  ...
  useEffect(() => {
    const endpoint = isChef ? 'teacher' : undefined;
    usersAPI.list(endpoint)
      .then(setUsers)
      .catch(console.error)
  ...
  const filtered = useMemo(() =>
    users.filter((u) => {
      ...
      const matchSearch = !search || `${u.first_name} ${u.last_name} ${u.email}`
              ...
              {filtered.map((u) => (
                <tr key={u.id} style={{ cursor: 'pointer' }} onClick={() => setSelectedUser(u)}>
                  ...
                  <td style={{ fontSize: 12, color: 'var(--text2)' }}>
                    {u.program_name || u.department_name || '—'}
                  </td>
        ...
        >
          <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: 'var(--text3)', margin: '3px 0' }}>
                {selectedUser.email}
              </div>
          ...
          <div className="grid-2">
            {[
              ['Statut',       selectedUser.status === 'pending' ? 'En attente' : 'Approuvé'],
              ...
              <div key={label}>
                <div style={{ fontSize: 10, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 3 }}>
                  {label}
                </div>
