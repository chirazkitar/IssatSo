...
import { statsAPI } from '../../api/apifetch';
...
export default function AdminStats() {
  ...
  if (!stats)  return (
    <div className="page">
      <EmptyState icon="" title="Erreur de chargement des statistiques" />
    </div>
  );

  const passRate = stats.gradeStats.passed + stats.gradeStats.failed > 0
    ? Math.round(stats.gradeStats.passed / (stats.gradeStats.passed + stats.gradeStats.failed) * 100)
    : 0;
  ...
  return (
    <div className="page">
      ...
      <div className="stats-grid">
        {[
          { glyph: '‍', value: stats.totals.students,          label: 'Étudiants',      color: 'var(--green)',   animClass: 'fade-up-1' },
          { glyph: '‍', value: stats.totals.teachers,          label: 'Enseignants',    color: 'var(--accent2)', animClass: 'fade-up-2' },
          { glyph: '',  value: stats.totals.programs,          label: 'Filières',       color: 'var(--gold)',    animClass: 'fade-up-3' },
          { glyph: '',  value: `${stats.gradeStats.avg}/20`,   label: 'Moyenne globale',color: 'var(--accent3)', animClass: 'fade-up-4' },
          { glyph: '',  value: `${passRate}%`,                  label: 'Taux de réussite',color:'var(--green)',   animClass: 'fade-up-5' },
          { glyph: '',  value: stats.gradeStats.failed,        label: 'Échecs',         color: 'var(--red)',     animClass: 'fade-up-5' },
        ].map((s) => (
      ...
      <div className="grid-2" style={{ marginBottom: 18 }}>
        ...
        <div className="card fade-up">
          <div className="card-header">
            <span className="card-title"> Synthèse Visuelle</span>
          </div>
          <div className="card-body">
            <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <DonutChart
                value={parseFloat(stats.gradeStats.avg).toFixed(1)}
                max={20}
      ...
      <div className="grid-2">
        ...
        <div className="card fade-up">
          <div className="card-header">
            <span className="card-title"> Performance par Module</span>
          </div>
          <div style={{ padding: 0 }}>
        ...
        <div className="card fade-up">
          <div className="card-header">
            <span className="card-title"> Derniers Inscrits</span>
          </div>
          <div style={{ padding: 0 }}>
