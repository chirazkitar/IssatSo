...
export default function DossierPage() {
  ...
  useEffect(() => {
    gradesAPI.dossier().then(setData).catch(console.error).finally(() => setLoading(false));
  }, []);

  if (loading) return <Loader text="Chargement du dossier..." />;
  if (!data)   return <div className="page"><EmptyState icon="xCircle" title="Impossible de charger le dossier" /></div>;
  ...
  const globalMent = getMentionFromGrade(globalAvg) || '—';
      ...
      <div className="grid-2" style={{ marginBottom:18 }}>
        ...
        <div className="card fade-up-2">
          ...
          <div className="card-body">
            ...
            <div style={{ marginTop:16, padding:14, background:'var(--bg3)', borderRadius:'var(--radius2)', textAlign:'center' }}>
              <div style={{ fontSize:11, color:'var(--text3)', marginBottom:4 }}>MOYENNE GÉNÉRALE</div>
              <div style={{ fontSize:28, fontWeight:800, color: parseFloat(globalAvg) >= 10 ? 'var(--green)' : 'var(--red)' }}>
                {globalAvg ?? '—'}/20
              </div>
              <div style={{ marginTop:6 }}>
                <MentionBadge mention={globalMent !== '—' ? globalMent : null} />
              </div>
            </div>
          </div>
        </div>
            ...
            <table style={{ width:'100%' }}>
              <thead>
                <tr style={{ background:'var(--bg3)' }}>
                  <th>Module</th><th>Coeff</th><th>Crédits</th>
                  <th>DS</th><th>Examen</th><th>Finale</th>
              ...
              <tbody>
                {sg.map((g) => (
                  <tr key={g.id} style={{ borderTop:'1px solid var(--border)' }}>
                    ...
                    <td style={{ fontWeight:700, fontSize:13, color: parseFloat(g.final_grade) >= 10 ? 'var(--green)' : 'var(--red)' }}>
